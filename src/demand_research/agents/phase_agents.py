...
class Phase2Agent(BasePhaseAgent):
    ...
    async def run(self, hypothesis: ProductHypothesis, phase1_result: PhaseResult) -> PhaseResult:
        ...
        research_prompt = (
            ...
            "Search Etsy/Gumroad/Notion reviews, Reddit (e.g. r/Etsy, "
            "r/smallbusiness, r/entrepreneur), YouTube comments, and forums for "
            "verbatim complaints and needs. Capture exact quotes with the source "
            "URL. Do not paraphrase a quote into something the person didn't say."
        )
        extract_instruction = (
